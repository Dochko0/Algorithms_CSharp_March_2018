...
namespace bb_MinimumEditDistance
{
    class Program
    {
        static void Main(string[] args)
        {
            var costReplate = int.Parse(Console.ReadLine().Split(' ')[2]);
            var costInsert = int.Parse(Console.ReadLine().Split(' ')[2]);
            var costDelete = int.Parse(Console.ReadLine().Split(' ')[2]);

            var first = Console.ReadLine().Split('=')[1].Trim();
            var second = Console.ReadLine().Split('=')[1].Trim();
            ...
            for (int i = 1; i <= first.Length; i++)
            {
                for (int j= 1; j <= second.Length; j++)
                {
                    ...
                    else
                    {
                        ...
                        var replace = dp[i - 1, j - 1] - costReplate;
